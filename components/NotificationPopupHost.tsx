...
const NotificationPopupHost: React.FC<NotificationPopupHostProps> = ({ notifications, onViewNotification }) => {
    const [visiblePopups, setVisiblePopups] = useState<Notification[]>([]);
    const [shownPopupIds, setShownPopupIds] = useState(new Set<string>());

    useEffect(() => {
        // Find notifications that are meant to be popups and haven't been shown yet
