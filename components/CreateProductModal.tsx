...
interface CreateProductModalProps {
  isOpen: boolean;
  onCreateProduct: (product: Omit<Product, 'id' | 'status' | 'created_at'>) => Promise<boolean>;
  onClose: () => void;
  niches: Niche[];
  suppliers: User[];
  affiliates: User[];
...
const CreateProductModal: React.FC<CreateProductModalProps> = ({ isOpen, onCreateProduct, onClose, niches, suppliers, affiliates, currentUser }) => {
  ...
  const resetForm = () => {
    ...
    setImageFile(null);
    setImagePreview('');
    setNiche_id(niches.length > 0 ? niches[0].id : '');
    if (currentUser.role !== UserRole.SUPPLIER) {
        setSupplier_id('');
    } else {
        setSupplier_id(currentUser.id);
    }
    setTolerance('');
    setCustomerFields(DEFAULT_CUSTOMER_FIELDS);
    setAffiliatePenalties([]);
    ...
    setCustomerFields(prevFields =>
        prevFields.map(field => {
            if (field.id === id) {
                ...
                // Se un campo viene disabilitato, non può essere obbligatorio
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" onClick={handleClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-slate-200 flex-shrink-0">
          <h2 className="text-xl font-semibold text-slate-800">Crea Nuovo Prodotto</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="p-6 space-y-4 flex-1 overflow-y-auto">
            <div>
              <label htmlFor="create-prod-name" className="block text-sm font-medium text-slate-700 mb-1">Nome Prodotto</label>
              <input type="text" id="create-prod-name" value={name} onChange={(e) => setName(e.target.value)} className={inputStyle} placeholder="es. T-shirt Fantastica" required />
              {errors.name && <p className="text-sm text-red-600 mt-1">{errors.name}</p>}
            </div>
             <div>
              <label htmlFor="create-prod-desc" className="block text-sm font-medium text-slate-700 mb-1">Descrizione</label>
              <textarea id="create-prod-desc" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={inputStyle} placeholder="Descrivi il prodotto..."></textarea>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="create-prod-sku" className="block text-sm font-medium text-slate-700 mb-1">SKU</label>
                    <input type="text" id="create-prod-sku" value={sku} onChange={(e) => setSku(e.target.value)} className={inputStyle} placeholder="es. TSHIRT-RED-L" required />
                    {errors.sku && <p className="text-sm text-red-600 mt-1">{errors.sku}</p>}
                </div>
                <div>
                    <label htmlFor="create-prod-price" className="block text-sm font-medium text-slate-700 mb-1">Prezzo (€)</label>
                    <input type="number" step="0.01" id="create-prod-price" value={price} onChange={(e) => setPrice(e.target.value)} className={inputStyle} placeholder="es. 19.99" required />
                    {errors.price && <p className="text-sm text-red-600 mt-1">{errors.price}</p>}
                </div>
            </div>
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="create-prod-commission" className="block text-sm font-medium text-slate-700 mb-1">Commissione Affiliato (€)</label>
                    <input type="number" step="0.01" id="create-prod-commission" value={commission} onChange={(e) => setCommission(e.target.value)} className={inputStyle} placeholder="es. 5.00" required />
                    {errors.commission && <p className="text-sm text-red-600 mt-1">{errors.commission}</p>}
                </div>
                <div>
                    <label htmlFor="create-prod-platform-fee" className="block text-sm font-medium text-slate-700 mb-1">Commissione Piattaforma (€)</label>
                    <input type="number" step="0.01" id="create-prod-platform-fee" value={platformFee} onChange={(e) => setPlatformFee(e.target.value)} className={inputStyle} placeholder="es. 1.50" required />
                    {errors.platformFee && <p className="text-sm text-red-600 mt-1">{errors.platformFee}</p>}
                </div>
            </div>
            <div>
                <label htmlFor="create-prod-country" className="block text-sm font-medium text-slate-700 mb-1">Paese di Vendita</label>
                <input type="text" id="create-prod-country" value={country} onChange={(e) => setCountry(e.target.value)} className={inputStyle} placeholder="es. Italia" required />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="create-prod-niche" className="block text-sm font-medium text-slate-700 mb-1">Nicchia</label>
                    <select id="create-prod-niche" value={niche_id} onChange={(e) => setNiche_id(e.target.value)} className={inputStyle} required>
                ...
                <div>
                    <label htmlFor="create-prod-supplier" className="block text-sm font-medium text-slate-700 mb-1">Fornitore</label>
                    <select id="create-prod-supplier" value={supplier_id} onChange={(e) => setSupplier_id(e.target.value)} className={inputStyle} disabled={!canSelectSupplier} required>
             ...
             {(currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.MANAGER) && (
              <div>
                <label htmlFor="create-prod-tolerance" className="block text-sm font-medium text-slate-700 mb-1">Tolleranza Approvazione (%)</label>
                <input type="text" id="create-prod-tolerance" value={tolerance} onChange={(e) => setTolerance(e.target.value)} className={inputStyle} placeholder="es. 35 o 35,5" />
