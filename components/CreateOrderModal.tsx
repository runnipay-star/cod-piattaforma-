...
interface CreateOrderModalProps {
  isOpen: boolean;
  onCreateOrder: (order: Omit<Order, 'id' | 'status' | 'date' | 'createdAt' | 'conversionId'>) => Promise<boolean>;
  onClose: () => void;
  products: Product[];
  affiliates: User[];
}

const CreateOrderModal: React.FC<CreateOrderModalProps> = ({ isOpen, onCreateOrder, onClose, products, affiliates }) => {
  ...
  const resetForm = () => {
    ...
    setQuantity(1);
    setAffiliateId(affiliates.length > 0 ? affiliates[0].id : '');
    setSubId('');
    setLoading(false);
    setErrors({});
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await onCreateOrder({
      ...
      quantity,
      totalPrice,
      subId: affiliateId ? subId.trim() : undefined,
    });
    setLoading(false);
    if (success) {
        onClose();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4" onClick={handleClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-slate-200 flex-shrink-0">
          <h2 className="text-xl font-semibold text-slate-800">Crea Ordine Manuale</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          <div className="p-6 space-y-4 flex-1 overflow-y-auto">
            ...
            <div>
              <label htmlFor="create-order-name" className="block text-sm font-medium text-slate-700 mb-1">Nome e Cognome</label>
              <input type="text" id="create-order-name" value={customerName} onChange={(e) => setCustomerName(e.target.value)} className={inputStyle} placeholder="es. Mario Rossi" />
              {errors.customerName && <p className="text-sm text-red-600 mt-1">{errors.customerName}</p>}
            </div>
            <div>
                <label htmlFor="create-order-phone" className="block text-sm font-medium text-slate-700 mb-1">Telefono</label>
                <input type="tel" id="create-order-phone" value={customerPhone} onChange={(e) => setCustomerPhone(e.target.value)} className={inputStyle} placeholder="es. 3331234567" />
                {errors.customerPhone && <p className="text-sm text-red-600 mt-1">{errors.customerPhone}</p>}
            </div>
            <div>
              <label htmlFor="create-order-address" className="block text-sm font-medium text-slate-700 mb-1">Indirizzo Completo</label>
              <input type="text" id="create-order-address" value={customerAddress} onChange={(e) => setCustomerAddress(e.target.value)} className={inputStyle} placeholder="es. Via Roma 1, 00100, Roma (RM)" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="create-order-product" className="block text-sm font-medium text-slate-700 mb-1">Prodotto</label>
                    <select id="create-order-product" value={productId} onChange={(e) => setProductId(e.target.value)} className={inputStyle} required>
                ...
                <div>
                    <label htmlFor="create-order-quantity" className="block text-sm font-medium text-slate-700 mb-1">Quantità</label>
                    <input type="number" min="1" id="create-order-quantity" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value, 10) || 1)} className={inputStyle} required />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="create-order-affiliate" className="block text-sm font-medium text-slate-700 mb-1">Affiliato</label>
                    <select id="create-order-affiliate" value={affiliateId} onChange={(e) => setAffiliateId(e.target.value)} className={inputStyle} required>
                ...
                {affiliateId && (
                    <div>
                        <label htmlFor="create-order-subid" className="block text-sm font-medium text-slate-700 mb-1">Sub ID (Opzionale)</label>
                        <input type="text" id="create-order-subid" value={subId} onChange={(e) => setSubId(e.target.value)} className={inputStyle} placeholder="es. sub1" />
                    </div>
                )}
            </div>
