...
const AffiliateForm: React.FC<AffiliateFormProps> = ({ affiliate, onSave, onClose }) => {
  ...
  useEffect(() => {
    if (affiliate) {
      ...
      setPrivacyPolicyUrl(affiliate.privacyPolicyUrl || '');
    } else {
      setName('');
      setEmail('');
      setPassword('');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const affiliateData: Partial<Affiliate> = { name, email, isBlocked, privacyPolicyUrl };
    if (password) {
  ...
  return (
    <form onSubmit={handleSubmit}>
        <div className="space-y-6">
            <div>
                <label htmlFor="name" className="block text-sm font-medium text-gray-700">Nome Affiliato</label>
            ...
            <div>
                ...
                <div className="relative mt-1">
                    <input 
                        ...
                        required={!affiliate} // Obbligatoria solo per i nuovi affiliati
        ...
        <div className="mt-8 flex justify-end gap-4">
            ...
            <button type="submit" className="bg-primary text-on-primary font-bold py-2 px-6 rounded-lg hover:bg-primary-dark transition-colors duration-200">
                {affiliate ? 'Salva Modifiche' : 'Crea Affiliato'}
            </button>
        </div>
    </form>
  );
